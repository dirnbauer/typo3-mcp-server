...
4. Call **`CreateSite`** with:
   ...
   - `defaultLanguage: { title: "English", locale: "en_US.UTF-8",
     iso-639-1: "en", flag: "us" }`
   - No `additionalLanguages` yet — we will add them later so we can
     verify both the create-then-add and replace-all flows.
5. Call **`GetPageTree`** again, `startPage: 0`, to confirm the new root
   appears. Capture the root page UID as `$ROOT`.

## Phase 2 — Build a small page tree

6. Use **`ListTables`** — confirm the schema layer is live and that you
   can see `pages`, `tt_content`, and `sys_file_reference`.
7. Use **`GetTableSchema`** for `pages` and review which fields are
   editable.
8. Create three pages under `$ROOT` with **`WriteTable`**
...
9. Call **`GetPage`** with `uid: $HOME` (or `url:
   "https://mcp-news-demo.ddev.site/"`) to confirm it resolves.
...
10. Call **`ImportFromUrl`** in `mode: "analyze"` first:
    - `url: "<chosen article URL>"`
    - `targetPid: $WORLD`
    - `mode: "analyze"`
    Review the proposed content elements. This should be read-only.
11. If the proposal looks sensible, call **`ImportFromUrl`** again with
    `mode: "execute"` — this creates a draft page with `tt_content`
    elements inside `$WORLD`. Capture the new page UID as `$ARTICLE`.
12. Call **`ReadTable`** (`table: "tt_content"`, `pid: $ARTICLE`) to list
    the created elements.
13. Call **`ImportContent`** to add a second, shorter plain-text summary
...
14. Call **`ListStorages`** to see available FAL storages and note the
    default upload storage.
15. Call **`BrowseFolder`** on the root folder of that storage, then
    **`BrowseFiles`** in an empty subfolder (or `/` of the sandbox) to
    confirm the file sandbox is configured.
16. For each image URL the article uses, call **`UploadFileFromUrl`**:
    - `url: "<image URL>"`
    - `targetPath: "mcp-demo/images/<slug>.jpg"`
    - `metadata: { title: "<image title>", alternative: "<alt text
      describing image>", description: "<short caption>", copyright:
      "© <outlet> — reproduced for demonstration under fair use" }`
17. Use **`SearchMedia`** with query `mcp-demo` to verify the uploaded
    files are indexed. Use **`SearchFile`** to find files by path
    fragment.
18. Call **`ReadFileMetadata`** on one of the uploads and confirm the
...
19. Also generate a small SVG logo with **`WriteFile`**:
    - `path: "mcp-demo/images/logo.svg"`
    - `content: "<svg xmlns='http://www.w3.org/2000/svg' ...>"`
    - `metadata: { title: "MCP News Demo Logo", alternative:
      "Placeholder logo", copyright: "CC0 — demo asset" }`
20. Upload a small binary file via **`UploadFile`** (base64 content) to
    confirm that path works too, e.g. a 1x1 transparent PNG at
    `mcp-demo/images/pixel.png`.
21. For each image uploaded in step 16 call **`AttachImage`**:
    - `table: "tt_content"`
...
22. Call **`CreateSite`** with `action: "addLanguage"`,
    `identifier: "mcp-news-demo"`, and:
    - `language: { title: "Deutsch", locale: "de_DE.UTF-8", iso-639-1:
      "de", flag: "de", base: "/de/", fallbackType: "fallback" }`
23. Repeat **`CreateSite addLanguage`** for Chinese:
    `{ title: "中文", locale: "zh_CN.UTF-8", iso-639-1: "zh",
    flag: "cn", base: "/zh/", fallbackType: "fallback" }`.
24. Repeat for Hebrew (right-to-left — still just another language entry):
    `{ title: "עברית", locale: "he_IL.UTF-8", iso-639-1: "he",
    flag: "il", base: "/he/", fallbackType: "fallback" }`.
25. Repeat for Spanish:
    `{ title: "Español", locale: "es_ES.UTF-8", iso-639-1: "es",
    flag: "es", base: "/es/", fallbackType: "fallback" }`.
26. Confirm that **`ListTables`** and **`GetTableSchema`** now expose the
    `sys_language_uid` parameter (the server only surfaces translation
    fields when the site is multilingual).
27. For `$HOME`, `$WORLD`, `$ABOUT`, and `$ARTICLE`, call **`WriteTable`**:
    - `table: "pages"`, `action: "translate"`, `uid: <live UID>`
    - `data: { sys_language_uid: "de", title: "<German title>",
      nav_title: "<German nav>", description: "<German meta>" }`
...
28. For each content element on `$ARTICLE`, call **`WriteTable`**
    (`table: "tt_content"`, `action: "translate"`, `uid: <element UID>`,
    `data: { sys_language_uid: "<iso>", header: ..., bodytext: ... }`)
    for all four languages.
29. Optional stress test: call **`BulkWrite`** with one batch containing
    up to 10 `translate` operations for tt_content elements at once
    (the tool caps at 50).

## Phase 6 — Copy, move, and reorganise content

30. Call **`CopyContent`** to duplicate one of the article's tt_content
    elements to `$ABOUT` (`sourceUid`, `targetPid: $ABOUT`).
31. Use **`WriteTable action: "move"`** to move the copied element to
    position `top` inside `$ABOUT`.
32. Use **`WriteTable action: "update"`** on the copy to change its
...
33. Call **`ContentAudit`** with `startPage: $ROOT`, `depth: 3`, and
    `checks: ["missingMetaDescription", "missingAltText",
    "missingSeoTitle"]`. Patch findings via **`WriteTable`** as needed.
34. Call **`Search`** with a phrase from the imported article to verify
    full-text search across tables returns the expected matches.
35. Call **`GetFlexFormSchema`** for a well-known plugin CType (e.g.
    `list` / `felogin_pi1` if present) to confirm FlexForm introspection
    works.
36. Call **`GetSystemLog`** with a recent time window to confirm the
    server can surface TYPO3's sys_log. Look for your own writes.

## Phase 8 — Redirects, site administration, extras

37. Call **`ManageRedirects`** with `action: "list"` and verify the
...
38. Call **`InstallExtension`** to install a small, well-known extension
...
39. Call **`SafeCli`** with a read-only `vendor/bin/typo3` command such
    as `cache:flush` (dry-run first) to exercise the sandboxed CLI.
40. If the x402 paywall surface is enabled, exercise
...
41. Call **`ListWorkspaces`** — confirm the MCP workspace still holds
    your changes.
42. Call **`WorkspaceReview`** with `workspaceId: <MCP workspace>` and
    scroll the diff. Everything from phases 1–8 should appear as
    pending.
43. Call **`PublishWorkspace`** with `workspaceId: <MCP workspace>` and
    `dryRun: true`. Read the report — no live rows change yet.
44. If the dry-run report is correct, call **`PublishWorkspace`** again
    with `dryRun: false` to publish everything to live.
45. After publishing, create one more throwaway change
    (e.g. `WriteTable` update on `$ABOUT` setting a silly title), then
    call **`RollbackWorkspace`** to verify the rollback path works.
46. Final **`GetPageTree`** with all four language flags to eyeball the
...
47. `WriteTable action: "delete"` on `$ARTICLE`, `$WORLD`, `$HOME`,
    `$ABOUT`, then `$ROOT`. Each delete stages in the workspace.
48. **`PublishWorkspace`** (dry-run, then live) to finalize.
49. Re-run **`GetPageTree`** — the demo tree should be gone.
50. `SafeCli` → `cache:flush` to remove any stale caches.
