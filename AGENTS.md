...
- File-write tools are restricted to the MCP file sandbox in strict mode; physical files are not workspace-versioned and therefore take effect immediately once written. The file sandbox, workspace-only-write rule, non-workspace table block, and outbound HTTP allowlist relax when `LocalModeService` reports DDEV / Development context — production should pin `localUnsafeMode=off` or `mcpServer.strictSandbox=1` if defense-in-depth requires it.
...
- Every bundled MCP tool should also have a `mcp:<name>` Symfony console command. Prefer a `GenericMcpToolCommand` service entry in `Configuration/Services.yaml`; use `AbstractMcpToolCommand` only for shortcuts that need custom options or formatting. CLI commands inherit `--json` / `--plain` / `--no-ansi` automatically.
