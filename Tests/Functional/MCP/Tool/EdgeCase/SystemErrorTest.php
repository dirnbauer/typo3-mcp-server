...
final class SystemErrorTest extends AbstractFunctionalTest
{
    ...
    public function testCorruptedTcaConfiguration(): void
    {
        ...
        try {
            ...
            if ($result->isError) {
                ...
                self::assertTrue(
                    str_contains((string)$errorText, 'configuration')
                    || str_contains((string)$errorText, 'tca')
                    || str_contains((string)$errorText, 'type')
                    || str_contains((string)$errorText, 'array')
                    || str_contains((string)$errorText, 'unexpected error occurred'), // New generic error message
    ...
    public function testTableAccessServiceFailure(): void
    {
        ...
        foreach ($edgeCaseTables as $table) {
            ...
            self::assertTrue($result->isError);
            self::assertStringContainsString('table', strtolower((string)$result->content[0]->text));
        }
    }
    ...
    public function testExtensionDependencyIssues(): void
    {
        ...
        try {
            ...
            if ($result->isError) {
                self::assertStringContainsString('workspace', strtolower((string)$result->content[0]->text));
            }
    ...
    public function testRaceConditions(): void
    {
        ...
        $data1 = json_decode((string)$read1->content[0]->text, true);
