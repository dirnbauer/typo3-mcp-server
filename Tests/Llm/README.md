...
1. Use PHP 8.2+ for this repository. In local development, DDEV is the safest
   option:

   ```bash
   ddev exec composer test:llm
   ```
...
4. Run a targeted test while iterating:
   ...
   ```bash
   ddev exec composer test:llm -- --filter testLlmFixesHeaderSpellingErrors
   ```
