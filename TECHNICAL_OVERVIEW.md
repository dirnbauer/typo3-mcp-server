...
The stdio transport (`vendor/bin/typo3 mcp:server`) runs in the
**operating-system account** that launches it. TYPO3 enforces editorial rules
for MCP tools; it does **not** isolate the PHP process from the rest of the
machine. If the MCP client also exposes a shell or terminal—or you wrap startup
in `bash`, `sh`, or similar—the effective risk can include **arbitrary host
commands** at that user’s privilege level (filesystem access, secrets in the
environment, processes and services outside TYPO3). Treat that combination like
interactive shell access: use it on **trusted local or non-production** hosts,
prefer dedicated OS accounts, and avoid pairing it with production credentials
or unrestricted client-side terminal capabilities. The same guidance appears for
operators in `README.md`, `Documentation/Installation/Index.rst` (local CLI
section), and the backend module **Local Setup (TYPO3 CLI)** tab.
...
- **AttachImage** - Download or reuse a sandbox `sys_file`, optionally run TYPO3 FAL `Image.CropScaleMask` (resize, crop, `fileExtension` conversion), then create `sys_file_reference` rows on a TCA file field (`append` or `replace`). Public HTTPS URLs (e.g. direct Unsplash image links) use the same stack as `UploadFileFromUrl`.
...
### URL Redirects
- **ManageRedirects** - Inspect `sys_redirect` records: list existing redirects with filters and explain redirect write limitations. TYPO3 keeps `sys_redirect` outside workspaces, so the tool does not create or delete live redirect rows through MCP.
...
- **ListPaidContent** - List pages gated by the optional x402 paywall extension, including price and description metadata.
- **GetPaidContent** - Return payment requirements or paid content for a specific page managed by the x402 paywall extension.
- **GetPaymentStats** - Summarize revenue, transaction counts, and grouped payment activity when the x402 payment-log table exists.

If the x402 extension surface is missing, these tools return configuration guidance instead of raw database failures.
...
1. **Remote clients** authenticate through the OAuth endpoints and call the
   MCP HTTP endpoint. `McpEndpoint` validates the token, sets up the backend
   user context, then calls the SDK's `HttpServerRunner::handleRequest()`
   directly and maps the full SDK response (headers, status, body) into a
   PSR-7 response. This ensures MCP protocol headers such as
   `Mcp-Session-Id` are forwarded correctly through TYPO3's middleware
   pipeline. Session state is persisted to `var/mcp_sessions/` via
   `FileSessionStore`.
2. **Local clients** start the stdio server through `vendor/bin/typo3 mcp:server`
   (OS-level trust boundary; see **Local stdio and the host OS boundary** above).
3. `McpServerFactory` builds the MCP server and registers the `tools/list`
   and `tools/call` JSON-RPC handlers.
4. `ToolRegistry` collects all Symfony DI services tagged `mcp.tool`. Native
   `ToolInterface` implementations are used directly; other tagged objects
   exposing `getName()` and `execute()` are wrapped in
   `CompatibleToolAdapter`, which normalizes their schema and result types.
   This lets third-party extensions contribute MCP tools without depending
   on this extension's interface.
...
## Important: use in controlled environments first

This version of the MCP server includes powerful tools that create sites, install
extensions, execute CLI commands, and publish workspace changes. Use it first in a
**local or staging TYPO3 instance** where you have full control — not on production
servers without reviewing and restricting the tool surface to match your security
requirements.
